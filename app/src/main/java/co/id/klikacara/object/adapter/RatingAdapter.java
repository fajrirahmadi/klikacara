...
public class RatingAdapter extends AbstractItem<RatingAdapter, RatingAdapter.ViewHolder> {
    ...
    public void bindView(@NonNull ViewHolder holder,
        ...
        if (StringUtils.isNotBlank(rating.getUrl()))
            GlideUtils.setFotoCircleFromStorage(holder.context,
                    StringHelper.getStringBuilderToString(BuildConfig.userDb, "/", rating.getUrl()),
                    holder.userImageView);
        else
            holder.userImageView.setImageResource(R.drawable.logo_klikacara);
        holder.nameTextView.setText(rating.getNama());
        holder.roleTextView.setText(String.valueOf(rating.getRating()));
        holder.ulasanTextView.setText(rating.getUlasan());
    }
